...
const TABLE: &str = "elura_schema_migrations";

pub(crate) async fn pending_postgres(pool: &PgPool, component: &str, target: i64) -> Result<bool> {
    sqlx::query(&format!(
        "CREATE TABLE IF NOT EXISTS {TABLE} (component VARCHAR(64) PRIMARY KEY, version BIGINT NOT NULL)"
    ))
    .execute(pool)
    .await
    .map_err(sql_error)?;
    let current =
        sqlx::query_scalar::<_, i64>(&format!("SELECT version FROM {TABLE} WHERE component=$1"))
            .bind(component)
            .fetch_optional(pool)
            .await
            .map_err(sql_error)?;
...
pub(crate) async fn pending_mysql(pool: &MySqlPool, component: &str, target: i64) -> Result<bool> {
    ...
    .map_err(sql_error)?;
    let current =
        sqlx::query_scalar::<_, i64>(&format!("SELECT version FROM {TABLE} WHERE component=?"))
            .bind(component)
            .fetch_optional(pool)
            .await
            .map_err(sql_error)?;
...
pub(crate) async fn record_postgres(pool: &PgPool, component: &str, version: i64) -> Result<()> {
    sqlx::query(&format!(
        "INSERT INTO {TABLE} (component,version) VALUES ($1,$2) ON CONFLICT (component) DO NOTHING"
    ))
    .bind(component)
    .bind(version)
    .execute(pool)
...
pub(crate) async fn record_mysql(pool: &MySqlPool, component: &str, version: i64) -> Result<()> {
    sqlx::query(&format!(
        "INSERT IGNORE INTO {TABLE} (component,version) VALUES (?,?)"
    ))
    .bind(component)
    .bind(version)
    .execute(pool)
